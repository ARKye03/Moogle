...
public class Data
{
    public Dictionary<string, List<int>> Vocabulary = []; // Este es el vocabulario del documento contra los indices de las palabras de dicho vocabulario
    public int MaxWordAppereance = 0; // Max word frzhcy 
    public double Module = 0; // Modulo del vector de pesos -> Valor |pesos|
    public Dictionary<string, double> pesos = []; // Peso de cada palabra en el documento
    public Data() { } //Bob
    ...
    { // Este es el metodo q calcula el peso de cada palabra en el documento y le da valor al Module del documento
        ...
        {
            pesos[par.Key] = Vocabulary[par.Key].Count / (double)MaxWordAppereance * GFiler[par.Key];
